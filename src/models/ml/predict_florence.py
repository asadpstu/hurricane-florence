#!/usr/bin/env python3
"""
ML Florence post-test improvement evaluation.
...
def main():
    ...
    if bundle.get("feature_columns")!=static_features:
        raise RuntimeError("Feature order mismatch between metadata and ML model.")
    if bundle.get("recursive_q_state") is not False:
        raise RuntimeError("ML model unexpectedly declares recursive Q state.")
    ...
    pred_path=a.output_dir/"ml_florence_predictions.csv"
    if pred_path.exists() and not a.overwrite:
        raise FileExistsError(f"{pred_path} exists. Use --overwrite.")
    atomic_csv(result,pred_path)
    atomic_csv(pd.DataFrame([metrics]),a.output_dir/"ml_florence_metrics.csv")
    atomic_json({
        "script_build":BUILD,
        "status":"PASS_ML_RETAINED_FLORENCE_POSTTEST_EVALUATION",
    ...
    },a.output_dir/"ml_florence_metadata.json")

    print("="*108)
    print(f"SCRIPT BUILD                       : {BUILD}")
    print("ML - FLORENCE POST-TEST EVALUATION")
    print("="*108)
    ...
    print("Status                             : PASS_ML_RETAINED_FLORENCE_POSTTEST_EVALUATION")
