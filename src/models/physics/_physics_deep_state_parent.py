...
def main():
    ...
    if "TIMING_AWARE" in BUILD:
        print(
            "Retained structure                 : "
            "deep-state baseflow + moisture-dependent preferential interflow"
        )
        print(
            "Calibration objective changed      : "
            "YES - EVENT BALANCED + CAL PEAK TIMING"
        )
        print(
            "Peak timing objective              : CALIBRATION EVENTS ONLY"
        )
        print(
            "Timing objective weight            : 0.200"
        )
        print(
            "Timing tolerance                   : 12.0 h"
        )
        print(
            "Timing scale                       : 60.0 h"
        )
        print(
            "Timing maximum penalty             : 2.000"
        )
        print(
            "Scoring-boundary peak penalty      : 0.500"
        )
    else:
        print(
    ...
    print(
        f"Additional calibrated parameters   : {len(NEW_PARAM_NAMES)}"
    )
    for name, bounds in zip(NEW_PARAM_NAMES, NEW_PARAM_BOUNDS):
        print(f"  {name:<35}: {bounds}")
    print("=" * 100)
    print()
    ...
    try:
        ...
        print(
            f"Physics metadata                    : {metadata_path}"
        )
