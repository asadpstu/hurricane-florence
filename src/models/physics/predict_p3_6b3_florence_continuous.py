#!/usr/bin/env python3
"""
Physics P3.6B3 Florence predictor — continuous antecedent-state run.
...
* Frozen P3.6B3 deep-groundwater parameters are used.
* No optimizer.
...
BUILD = "PHYSICS_P3_6B3_FLORENCE_CONTINUOUS_ANTECEDENT_V1"
...
def main():
    ...
    # ------------------------------------------------------------------
    # Build the frozen P3.6B3 runtime directly from the recovered deep-state
    # parent.  Do NOT use physics_core.py here: that is the later timing-aware
    # moisture/interflow branch and has a different parameter vector.
    #
    # We intentionally bypass _load_deep_state(), because that development
    # helper rejects 2018+ data.  Florence deep wetness is loaded below and
    # used only for the initial state; observed Q never enters the simulation.
    # ------------------------------------------------------------------
    runtime = load_local_module(
        "_physics_deep_state_parent.py",
        "physics_p3_6b3_runtime_for_florence",
    )

    parent, parent_path, parent_build = runtime._load_parent()

    original_names = list(parent.PARAM_NAMES)
    original_bounds = list(parent.PARAM_BOUNDS)

    extended_names, extended_unpack = runtime._make_unpack_params(
        original_names
    )

    parent.PARAM_NAMES = extended_names
    parent.PARAM_BOUNDS = (
        original_bounds + list(runtime.NEW_PARAM_BOUNDS)
    )
    parent.unpack_params = extended_unpack
    simulate_event = runtime._make_simulate_event(parent)
    ...
    if missing_params:
        raise RuntimeError(
            f"Frozen P3.6B3 parameter file missing: {missing_params}"
        )
    ...
    if not np.isfinite(x).all():
        raise RuntimeError(
            "Frozen P3.6B3 parameters contain non-finite values."
        )
    ...
    event = {
        "event_block_id": "FLORENCE_2018_P3_6B3_CONTINUOUS",
    ...
    print("=" * 108)
    print(
        "PHYSICS P3.6B3 FLORENCE — CONTINUOUS ANTECEDENT RUN"
    )
    print("=" * 108)
    print(f"Build                              : {BUILD}")
    print(f"Parent                             : {parent_build}")
    print(f"P3.6B3 runtime parent              : {parent_path}")
    ...
    pred_path = (
        a.output_dir
        / "physics_florence_predictions.csv"
    )
    daily_path = (
        a.output_dir
        / "physics_florence_daily.csv"
    )
    metric_path = (
        a.output_dir
        / "physics_florence_metrics.json"
    )
    ...
    metadata = {
        "build": BUILD,
        "model": "Physics P3.6B3 deep-groundwater initialization",
