"""
Production Hurricane Florence full-AOI longitudinal-WSE inundation mapper.
...
surface:
    ...
    Model discharge -> limb-aware Q->H stage -> NAVD88 WSE at gauge
    -> longitudinal-only mainstem WSE profile -> full-AOI USGS 3DEP DEM
    -> mainstem-connected wet cells -> depth = WSE - DEM

No satellite flood footprint or NOAA FIM footprint is used to create the full-AOI production flood map.
...
BUILD = "FLORENCE_FULL_AOI_LONGITUDINAL_WSE_INUNDATION"
...
def parse_args():
    ...
    p.add_argument("--source-name", default="Model")
    p.add_argument("--q-to-h", type=Path, required=True)
    p.add_argument(
        "--dem",
        type=Path,
        required=True,
        help="USGS 3DEP DEM in metres NAVD88.",
    )
    ...
    p.add_argument(
        ...
        default=[],
        help=(
            "Optional exact diagnostic timestamp; may be repeated. The nearest "
            "model row is mapped in addition to daily maxima. Example: "
            "2018-09-18T15:45:05Z for a diagnostic state."
        ),
    )
    p.add_argument(
...
def _read_prediction_frame(path):
    ...
    if df.empty:
        raise RuntimeError("Prediction CSV has no finite discharge rows.")
    return df
...
def read_mapping_states(path, start, days, snapshot_times, max_snapshot_offset_minutes):
    ...
    for value in snapshot_times:
        ...
        if offset_min > max_snapshot_offset_minutes:
            raise RuntimeError(
                f"Nearest model state to {target.isoformat()} is {offset_min:.1f} min away; "
...
def main():
    a = parse_args()
    require([a.predictions, a.q_to_h, a.dem, a.mainstem_gpkg])
    if a.daily_days < 1:
    ...
    rows_out = []
    with rasterio.open(a.dem) as dem_ds:
        if dem_ds.crs is None:
            raise RuntimeError("1 m DEM has no CRS.")
        res_x = abs(dem_ds.transform.a)
        res_y = abs(dem_ds.transform.e)
        if max(abs(res_x - res_y), 0.0) > 0.05:
            raise RuntimeError(
                f"Expected projected production DEM, got {res_x:.3f} x {res_y:.3f} m. "
    ...
    peak_inundation_path = a.output_dir / "peak_flood_extent.tif"
    peak_depth_path = a.output_dir / "peak_depth_m.tif"
    peak_wse_path = a.output_dir / "peak_wse_navd88_m.tif"
    ...
    metadata = {
        ...
        "method": {
            "forcing": f"{a.source_name} predicted discharge",
        ...
        "limitations": [
            ...
            "This terrain-based method is an approximation and should be interpreted with its stated hydraulic/terrain limitations.",
